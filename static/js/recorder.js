...
(function () {
  ...
  async function showAndRecord(idx) {
    ...
    if (mode === "full" || !hasReferenceAudio(lang)) {
      ...
      playWordAudio(main, lang);   // fire-and-forget; mic records along with it
    } else {
      await new Promise(r => setTimeout(r, WORD_SHOW_MS));
      await playWordAudio(main, lang);
      startRecording();
    }
  }
  ...
  startBtn.addEventListener("click", async () => {
    ...
    // Default mode: random column per row for diverse phonetic coverage.
    // Full (developer) mode: keep fixed-column subsection methodology.
    try { words = getSessionWords(subsection, lang, safeSpeaker, mode === "default"); }
